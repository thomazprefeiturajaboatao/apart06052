...
const CashAccrualCompareReport: React.FC<Props> = ({
    ...
    const { autoMap, manualMap } = useMemo(() => {
        ...
        (manualConciliations || []).forEach(mc => {
            // Para casos comuns: 1 conciliação manual -> N reservas / 1+ depósitos.
            // Como pode haver vários depósitos, escolho o primeiro que existir como referência de data.
            const refDepositId = mc.depositIds[0];
            const refDeposit = (deposits || []).find(d => d.id === refDepositId);
            if (!refDeposit) return;
            mc.reservationIds.forEach(rid => {
                manualMap.set(rid, { deposit: refDeposit, siblingReservationIds: mc.reservationIds });
            });
        });
    ...
    }, [monthlyReservations, autoMap, manualMap, reservations, selectedYear]);
